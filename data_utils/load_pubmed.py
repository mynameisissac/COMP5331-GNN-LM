...
def get_pubmed_casestudy(corrected=False, SEED=0):
    ...
    # load data
    data_name = 'PubMed'
    # path = osp.join(osp.dirname(osp.realpath(__file__)), ROOT)
    dataset = Planetoid(f'{ROOT}', data_name, transform=T.NormalizeFeatures())
    data = dataset[0]
...
def get_raw_text_pubmed(use_text=False, seed=0):
    ...
    f = open(f'{ROOT}/PubMed_orig/pubmed.json')
    pubmed = json.load(f)
